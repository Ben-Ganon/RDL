...
def run():
    ...
    baseline = ValueNetwork(state_size, learning_rate_baseline)
    ...
    with tf.Session() as sess:
        ...
        for episode in range(max_episodes):
            ...
            for step in range(max_steps):
                ...
                episode_transitions.append(Transition(state=state, action=action_one_hot, reward=reward, next_state=next_state, done=done, value=value))
            ...
            for t, transition in enumerate(episode_transitions):
                total_discounted_return = sum(discount_factor ** i * t.reward for i, t in enumerate(episode_transitions[t:])) - transition.value # Rt - V(s)
